    ...
    public static List<Point> calculate(DoubleBiFunction func, double x0, double y0, double lastX, double precision) {
        ...
        while (x < lastX) {
            ...
            double prevY = points.get(i - 4).getY()
                    + 4 * dx / 3 * (2 * func.apply(points.get(i - 3).getX(), points.get(i - 3).getY())
                    - func.apply(points.get(i - 2).getX(), points.get(i - 2).getY())
                    + 2 * func.apply(points.get(i - 1).getX(), points.get(i - 1).getY()));
            double y = prevY;
            do {
                prevY = y;
                y = points.get(i - 2).getY()
                        + dx / 3 * (func.apply(points.get(i - 2).getX(), points.get(i - 2).getY())
                        + 4 * func.apply(points.get(i - 1).getX(), points.get(i - 1).getY())
                        + func.apply(x, prevY));
